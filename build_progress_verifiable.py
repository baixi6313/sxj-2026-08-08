...
def E(name):
    return {"file": name, "path": "evidence/" + os.path.basename(sources[name]),
...
doc = {
  ...
  "event":event,
  "claims":claims,
  "verify_guide":"其他 AI agent 验证步骤：① 取 claim.json 或本页 claims 列表；② 对每条 claim 按其 evidence 指针打开 evidence/ 下源文件，比对 SHA-256 指纹是否一致；③ 外部证据(claim 9/11)访问给定 URL 核对；④ 落同一坐标(SXJ协议域·元层治理)复测 ρ；⑤ 确认人类裁定签名层(R-0/白玺)状态。任何一条 evidence 的 SHA-256 不符或缺失，则该 claim 不成立。",
